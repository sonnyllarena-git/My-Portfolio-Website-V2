...
| Object storage     | Cloudflare R2 (@aws-sdk/client-s3) | 3.1127.0                    | Explicit approval (2026-09-08): backs admin video/audio uploads for the Music Lab app (Phase, this entry). Render's disk is ephemeral and the files are too large for base64-in-Postgres, so R2's free tier (10GB, zero egress) stores them instead; accessed via the S3-compatible API.                              |
