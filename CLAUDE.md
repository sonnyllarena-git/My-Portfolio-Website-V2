...
| Backend             | Express                               | 5.2.1                       | Added in Phase 76 to back the Store admin catalog (Draft→Preview→Publish); Phase 98 adds a `resumeTemplates` table to the same server; Phase 100 adds a `leaderboardScores` table for the global arcade leaderboard.                                                                                                                    |
| Database            | PostgreSQL (via `pg`)                 | pg 8.23.0                   | Migrated off better-sqlite3 (2026-09-07) so the DB survives Render's ephemeral filesystem; same Postgres instance now also backs the arcade leaderboard (Phase 100, explicit approval 2026-09-08) instead of adding a second DB layer.                                                                                                  |
...
| PDF export          | html2canvas-pro                       | 2.4.0                       | Explicit approval (2026-08-31): renders the Resume Generator's live preview to a canvas so it can be embedded in a downloaded PDF. The original `html2canvas` (1.4.1, unmaintained for this) can't parse Tailwind v4's `oklab()` colors and threw on every render — swapped for this maintained, API-compatible fork.                   |
| PDF export          | jspdf                                 | 4.2.1                       | Same approval as above — turns the html2canvas-pro snapshot into a real one-click multi-page PDF download (`window.print()` alone can't skip the OS print dialog).                                                                                                                                                                      |
| Object storage      | Supabase Storage (@aws-sdk/client-s3) | 3.1127.0                    | Explicit approval (2026-09-08): backs admin video/audio uploads for the Music Lab app. Swapped from Cloudflare R2 the same day — R2 requires a card on file even for free-tier usage, Supabase Storage's free tier (1GB, 2GB egress/month) does not; still accessed via the S3-compatible API, so the same client library carries over. |
| Email notifications | Resend                                | REST API (no SDK)           | Explicit approval (2026-09-09): notifies Sonny by real email when a visitor submits the Gmail app's Contact Inquiry form (Phase 110). Free tier (100 emails/day), called via plain `fetch` to Resend's REST API in `backend/resendClient.js` rather than installing their SDK, so no new npm dependency.                                |
