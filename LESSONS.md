...
- [2026-08-25] Found `StartMenu.jsx`'s P514/P515 tasks marked `[x]` in TASKS.md, but the code on disk used a different, unspecced technique (button growing `w-9` → `hover:w-40` so the label was hidden until hover) than the plan's described `group`/sliding-`scale-x`-highlight with an always-visible label on desktop — an uncommitted edit from a session that ended before the plan's own pass condition was re-checked against the result → a `[x]` in TASKS.md means a prior session believed it was done, not that the code matches the task's description; when picking up a task that's already checked off, diff the actual code against the pass condition before trusting it, especially for uncommitted changes.
